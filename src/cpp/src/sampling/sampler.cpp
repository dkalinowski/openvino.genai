...
SamplerOutput Sampler::sample(const std::vector<SequenceGroup::Ptr> & sequence_groups,
    ...
    for (auto& sequence_group : sequence_groups) {
        ...
        const bool could_generate_before = sequence_group->can_generate_tokens();
        sequence_group->finish_iteration();
        // Notify handle when prefill completes (can_generate_tokens transitions from false to true)
        if (!could_generate_before && sequence_group->can_generate_tokens()) {
