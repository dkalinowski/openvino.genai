...
ContinuousBatchingPipeline::Eagle3DecodingImpl::add_request(uint64_t request_id,
    ...
    m_draft_generations.insert({request_id, m_draft_pipeline->add_request(request_id, draft_input_ids, draft_sampling_params, token_type_ids, position_ids, rope_delta)});
    return m_main_pipeline->add_request(request_id, input_ids, sampling_params, token_type_ids, position_ids, rope_delta);
}
